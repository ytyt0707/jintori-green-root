...
| `JET_CHARGE_TIME` | 20000 ms | ジェットチャージ時間（ブースト未使用で昇格） |
...
### ブースト → ジェット レベルアップシステム

ブースト使用可能状態で20秒間発動せずに待つと、ジェットに自動昇格する。

| 項目 | ブースト | ジェット |
|:-----|:---------|:---------|
| 速度倍率 | 1.8倍 | 3.6倍 (1.8 × 2) |
| 持続時間 | 2000 ms | 2000 ms |
| クールダウン | 5000 ms | 5000 ms |
| 発動条件 | クールダウン完了 | ブースト未使用20秒経過 |
| ボタン表示 | 緑「⚡BOOST」 | オレンジ脈動「🚀 JET」 |
| エフェクト | レインボー + スピードライン | 炎オーラ + 二重リング + 高速ライン |

### フロー

```
スポーン → 5秒クールダウン → ブースト使用可能（緑ボタン）
   ├─ すぐ発動 → 通常ブースト（1.8倍）→ 5秒クールダウン → ...
   └─ 20秒待機 → ジェット昇格（オレンジボタン）
        └─ 発動 → ジェット（3.6倍）→ 5秒クールダウン → ...
```

### チャージ中の表示

チャージ中（0〜20秒）はボタンが緑→オレンジのプログレスバーで進行を表示:
「⚡BOOST 🚀15s」のように残り秒数を表示。

### サーバー側プロパティ

| プロパティ | 型 | 説明 |
|:-----------|:---|:-----|
| `p.boostUntil` | timestamp | ブースト終了時刻 |
| `p.boostCooldownUntil` | timestamp | クールダウン終了時刻 |
| `p.jetUntil` | timestamp | ジェット終了時刻 |
| `p.boostReadySince` | timestamp | ブースト使用可能になった時刻（ジェットチャージ起点） |
| `p.jetting` | boolean | ジェット発動中フラグ |
...
// ブースト/ジェット発動: 2バイト（サーバーがチャージ状態に応じて判定）
...
if (msg.length === 2 && msg[1] === 1) {
    if (!p.boostCooldownUntil || now >= p.boostCooldownUntil) {
        ...
        if (jetReady) {
            p.jetUntil = now + BOOST_DURATION;   // ジェット発動
        } else {
            p.boostUntil = now + BOOST_DURATION; // 通常ブースト
        }
...
### 通信プロトコル（自分のプレイヤー向け追加フィールド）

| フィールド | 型 | 説明 |
|:-----------|:---|:-----|
| `bs` | int | ブースト/ジェット残り時間 (100ms単位) |
| `bc` | int | クールダウン残り秒数 |
| `mb` | 1 | ジェット発動中フラグ |
| `jc` | int (0-20) | ジェットチャージ秒数（チャージ中のみ送信） |

### クライアント表示状態

| 状態 | ボタン表示 |
|:-----|:-----------|
| ジェット発動中 | 炎グラデーション「🚀 JET!!」 |
| 通常ブースト発動中 | レインボーグラデーション「⚡ BOOST!」 |
| クールダウン中 | グレー + プログレスバー + 残り秒数 |
| ジェットREADY（20秒チャージ完了） | オレンジ脈動ボタン「🚀 JET」 |
| チャージ中（1〜19秒） | 緑→オレンジプログレス「⚡BOOST 🚀Ns」 |
| 通常ブースト使用可能 | 緑ボタン「⚡BOOST」 |
...
5. 敵アイランド処理:
   ...
   - 最大アイランド ≤ 10セル or 幅/高さ ≤ 5グリッド: 全てキャプチャ
6. 空白アイランド: 同様のロジック
7. キル判定: プレイヤー位置Mapで O(1) ルックアップ
8. 所有権変更 + スコア更新
9. キル時陣地一括奪取: Set(kills) + worldGrid直接スキャン
10. territoryRects再構築（1回のみ）
```

### 狭小領域の全埋め判定

敵/空白アイランド処理において、最大アイランドのバウンディングボックスが幅5グリッド以下 or 高さ5グリッド以下の場合、半分ずつ削る処理ではなく全アイランドを一括キャプチャする。細長い陣地を横断した際の直感的な挙動を実現。

### テリトリー没収
- キルされたプレイヤーのテリトリーは攻撃者に移転（全消去ではない）
- トレイルカットキル時: 被害者のテリトリー所有権が攻撃者に変更
- 複数キル時も `Set(hitTargets)` でworldGridを1回スキャンして一括奪取

### 最適化 (v12追加)
- **早期スキップ**: キャプチャループで `baseGrid[idx]===1` / `visitedCur[idx]===1` をチェックし、自陣・外部到達可能セルをスキップ（領域70%充填時に約63,000セル省略）
- **キル検出Map化**: `Object.values(players).forEach()` をグリッド座標Map事前構築に置換。O(セル数×プレイヤー数) → O(セル数)
- **rebuild統合**: attemptCapture内の `rebuildTerritoryRects()` 呼び出しを最大4回→1回に統合
- **ライン切断奪取統合**: server.v5.jsの軌跡カットkill後の陣地奪取もループ外で一括処理 + rebuild1回
...
| # | 最適化 | 対象ファイル | 効果 |
|:--|:-------|:-------------|:-----|
| 22 | rebuildTerritoryRects呼び出し統合: 最大4回→1回 | game.js, server.v5.js | ラグスパイク50-70%減 |
| 23 | キャプチャループ内キル検出Map化: O(セル×人数)→O(セル) | game.js | 10倍以上高速化 |
| 24 | キャプチャループ早期スキップ: baseGrid/visitedCurチェック | game.js | 領域70%時63,000セルスキップ |
| 25 | チーム旗クラスタリング: O(N²) 3重ループ→O(N) Union-Find | game.js | 5,000rect時 ~50ms→<5ms |
| 26 | ミニマップ圧縮レベル: deflateSync level 6→1 | game.js | 圧縮処理軽量化（リスクゼロ） |
| 27 | ライン切断陣地奪取統合: ループ内個別rebuild→ループ外一括 | server.v5.js | 複数kill時のrebuild回数削減 |

**#22 詳細: rebuildTerritoryRects統合**

従来、`attemptCapture()` 内で以下の3箇所で `rebuildTerritoryRects()` が呼ばれていた:
1. キャプチャ成功直後 (game.js)
2. kill毎の陣地奪取後 (game.js, forEachループ内)
3. kill処理完了後 (game.js)

加えて `server.v5.js` の軌跡カットkill後にもセグメントループ内で個別に呼ばれていた。
各呼び出しで300×300=90,000セルのフルグリッドスキャンが発生。

改善後: kill時の陣地奪取を `Set(kills)` + `worldGrid` 直接スキャンに変更し、
`rebuildTerritoryRects()` は全処理完了後に1回だけ呼び出す。

**#25 詳細: Union-Findクラスタリング**

チーム旗の位置計算で使用していた3重ネストクラスタリング:
```
while(changed) → rectList.forEach() → cluster.rects.forEach() → Math.hypot()
```
を空間グリッド（セルサイズ100px）+ Union-Find（path compression + union by rank）に置換。
`generateMinimapBitmap()` と `calculateMapFlags()` で共通関数 `clusterRectsUnionFind()` を使用。
...
- **ジェット機能（ブースト→ジェット レベルアップ）**: ブースト使用可能状態で20秒間待機すると自動的にジェットに昇格。ジェットは3.6倍速（通常ブーストの2倍）。ボタンUIで緑→オレンジのプログレス表示、チャージ完了時は脈動エフェクト付きオレンジボタンで視覚的にフィードバック。通信プロトコルに`jc`（ジェットチャージ秒数）フィールドを追加
- **狭小領域の全埋め判定**: 敵/空白アイランド処理で、最大アイランドの幅or高さが5グリッド以下の場合は半分ずつ削る処理ではなく全アイランドを一括キャプチャ。細長い陣地を横断した際に直感的に全取得可能に

### パフォーマンス改善（領域充填時のラグ対策）

- **rebuildTerritoryRects呼び出し統合**: attemptCapture内の最大4回の呼び出しを1回に統合。ライン切断kill後の陣地奪取も一括処理に変更。90,000セル×4回スキャンを90,000セル×1回に削減
- **キャプチャループ早期スキップ**: `baseGrid`/`visitedCur`チェックで自陣・外部到達可能セルをスキップ。領域70%充填時に約63,000セル（全体の70%）を省略
- **キル検出Map化**: キャプチャループ内のプレイヤー位置検索を`Object.values(players).forEach()`からグリッド座標Map O(1)ルックアップに変更
- **チーム旗クラスタリングO(N)化**: O(N²)の3重ネストクラスタリングを空間グリッド+Union-Find（path compression + union by rank）に置換。`clusterRectsUnionFind()`共通関数として`generateMinimapBitmap`と`calculateMapFlags`で共有。5,000rect時の処理時間を~50ms→<5msに削減
- **ミニマップ圧縮レベル低減**: deflateSync level 6→1（30×30=900バイトに対して過剰だったため）

### 設定変更

- `highSpeedEvent`: `true` → `false`（ブースト祭りイベント終了）
- `JET_CHARGE_TIME`: 20000ms（新規追加）
